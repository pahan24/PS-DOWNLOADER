...
function dlBin(cb) {
  ...
  console.log('[ytdl] downloading binary...');
...
function run(args, cb) {
  getYT(function(e, bin) {
    if (e || !bin) return cb(new Error('Media engine not ready. Wait 30s and try again.'));
...
/* warm up */
getYT(function(e, b) {
  if (e) console.log('[ytdl] warmup failed:', e.message);
  else   console.log('[ytdl] ready at', b);
});
...
var LADDER = [
  {h:1080,q:'1080p',tag:'FHD',i:'🎬'},
  {h:720, q:'720p', tag:'HD', i:'📹'},
  {h:480, q:'480p', tag:'SD', i:'📺'},
  {h:360, q:'360p', tag:'SD', i:'📺'},
  {h:240, q:'240p', tag:'LOW',i:'📱'},
  {h:144, q:'144p', tag:'LOW',i:'📱'}
];
...
function fmts(info, url) {
  ...
  LADDER.forEach(function(q) {
    ...
    out.push({
      id:'mp4-'+q.h, label:q.q+' (.mp4)', type:'video', ext:'mp4',
      quality:q.q, size:m?sz(m.filesize||m.filesize_approx):null,
      fmtArg:'bestvideo[height<='+q.h+'][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<='+q.h+']+bestaudio/best[height<='+q.h+']',
      tag:q.tag, icon:q.i, available:av
    });
  });
  ...
  var ar = raw.filter(function(f){ return f.vcodec==='none'&&f.acodec&&f.acodec!=='none'; })
...
app.post('/api/info', function(req, res) {
  ...
  run([
    ...
    var info;
    try { info = JSON.parse(stdout); } catch(x) { return res.status(500).json({error:'Parse error.'}); }
    var id = ytid(url);
    res.json({
      success: true, platform: plt(url),
      media: {
        title:     info.title||'Untitled',
        thumbnail: id ? 'https://i.ytimg.com/vi/'+id+'/hqdefault.jpg' : (info.thumbnail||null),
        duration:  dur(info.duration),
        uploader:  info.uploader||info.channel||null,
        viewCount: info.view_count ? Number(info.view_count).toLocaleString() : null,
        formats:   fmts(info, url)
      }
    });
  });
});
